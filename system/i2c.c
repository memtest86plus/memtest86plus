...
{
    uint8_t spdidx = 0, spd_line_idx = 0, max_slot_count = 0;
    ...
    for (spdidx = 0; spdidx < max_slot_count; spdidx++) {
...
static uint8_t __attribute__((optimize("O0"))) i2c_read_byte(uint8_t *base, uint8_t dev_addr, uint16_t offset)
{
    uint8_t buf;
