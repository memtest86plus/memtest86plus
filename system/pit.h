...
static inline unsigned pit_freq2div(unsigned frequence)
{
	return (0.5f + 1193180.0f/frequence);
}


static inline void pit_off(void)
{
	unsigned x = inb(PIT_STATUS_PORT) & ~PIT_STATUS_MASK;
 
	outb(x, PIT_STATUS_PORT);
}


static inline void pit_init_square_wave_generator(unsigned ch, unsigned frequence)
{
	unsigned clk_div = pit_freq2div(frequence);

	unsigned cfg = PIT_CTL_OP_MODE3 | PIT_CTL_BIN16_MODE | PIT_CTL_ACCESS_LBYTE_HBYTE | _PIT_CTL_CH(ch);

	unsigned PIT_DATA_PORT = PIT_CH0_PORT + ch;    // base + offset
	...
	unsigned x = inb(PIT_STATUS_PORT);
